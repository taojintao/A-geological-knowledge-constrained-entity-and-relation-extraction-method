...
def add_relation_tokens(tokenizer, rel_labels):
    new_tokens = []
    for label in rel_labels:
        new_tokens.append('#%s#'%label)
    tokenizer.add_tokens(new_tokens)
    logger.info('# vocab after adding relations: %d'%len(tokenizer))
...
def getPredicate(schemas_df,subject_type,object_type):
    '''
    Retrieve relationships from schema based on the type of the subject and object
    '''
    select=schemas_df.loc[(schemas_df['subject_type']==subject_type)&(schemas_df['object_type']==object_type)]
    predicate=None
    if select.shape[0] !=0:
        predicate=select.iloc[:,1].tolist() #['指示作用', '属性关系']
    return predicate


def convert_examples_to_features(examples, label2id, max_seq_length, tokenizer, special_tokens, schemas_df, unused_tokens=True, using_potential_predicates=True):
    """
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        predicates=getPredicate(schemas_df,example['subj_type'],example['obj_type'])
        pps=[]
        if predicates:
            for predicate in predicates:
                pp = get_special_token('#%s#'%predicate) #potential predicates
                pps.append(pp)
        ...
        if using_potential_predicates:
            len_tokens_pp = len(pps)
            if len_tokens_pp > 0:
                for index, pp in enumerate(pps):
                    tokens.insert(index+1, pp)
                sub_idx += len_tokens_pp
                obj_idx += len_tokens_pp
...
def print_pred_json(eval_data, eval_examples, preds, id2label, output_file):
    ...
    with open(output_file, 'w',encoding='utf-8') as f:
        f.write('\n'.join(json.dumps(doc,ensure_ascii=False) for doc in js))
...
def main(args):
    ...
    schemas_df=pd.read_excel(args.schema_file)

    # train set
    if args.do_train:
        train_dataset, train_examples, train_nrel = generate_relation_data(args.train_file, use_gold=True, context_window=args.context_window, schemas_df=schemas_df)
    # dev set
    if (args.do_eval and args.do_train) or (args.do_eval and not(args.eval_test)):
        eval_dataset, eval_examples, eval_nrel = generate_relation_data(os.path.join(args.entity_output_dir, args.entity_predictions_dev), use_gold=args.eval_with_gold, context_window=args.context_window, schemas_df=schemas_df)
    # test set
    if args.eval_test:
        test_dataset, test_examples, test_nrel = generate_relation_data(os.path.join(args.entity_output_dir, args.entity_predictions_test), use_gold=args.eval_with_gold, context_window=args.context_window, schemas_df=schemas_df)
    ...
    if os.path.exists(os.path.join(args.output_dir, 'label_list.json')):
        with open(os.path.join(args.output_dir, 'label_list.json'), 'r', encoding='utf-8') as f:
            label_list = json.load(f)
    else:
        label_list = [args.negative_label] + task_rel_labels[args.task]
        with open(os.path.join(args.output_dir, 'label_list.json'), 'w', encoding='utf-8') as f:
    ...
    #tokenizer = AutoTokenizer.from_pretrained(args.bert_model_dir, do_lower_case=args.do_lower_case)
    tokenizer = BertTokenizer.from_pretrained(args.bert_model_dir, do_lower_case=args.do_lower_case)
    ...
    if os.path.exists(os.path.join(args.output_dir, 'special_tokens.json')):
        with open(os.path.join(args.output_dir, 'special_tokens.json'), 'r', encoding='utf-8') as f:
    ...
    if args.do_eval and (args.do_train or not(args.eval_test)):
        eval_features = convert_examples_to_features(
            eval_examples, label2id, args.max_seq_length, tokenizer, special_tokens, schemas_df, unused_tokens=not(args.add_new_tokens))
        logger.info("***** Dev *****")
    ...
    with open(os.path.join(args.output_dir, 'special_tokens.json'), 'w', encoding='utf-8') as f:
        json.dump(special_tokens, f, ensure_ascii=False)

    if args.do_train:
        train_features = convert_examples_to_features(
            train_examples, label2id, args.max_seq_length, tokenizer, special_tokens, schemas_df, unused_tokens=not(args.add_new_tokens))
        ...
        model = RelationModel.from_pretrained(args.bert_model_dir, num_rel_labels=num_labels)
    ...
    if args.do_eval:
        ...
        if args.eval_test:
            ...
            eval_features = convert_examples_to_features(
                test_examples, label2id, args.max_seq_length, tokenizer, special_tokens, schemas_df, unused_tokens=not(args.add_new_tokens))
            eval_nrel = test_nrel
            logger.info(special_tokens)
            logger.info("***** Test *****")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task', type=str, default='geodata', choices=['ace04', 'ace05', 'scierc', 'geodata'])
    parser.add_argument("--model", default='bert-base-uncased', type=str)
    parser.add_argument('--bert_model_dir', type=str, default='/pretrained/bert-base-chinese',
                    help="the base model directory")
    parser.add_argument("--output_dir", default="/RE/output/relation_output", type=str,
    ...
    parser.add_argument("--do_train", action='store_true', default=1, help="Whether to run training.")
    parser.add_argument("--train_file", default="/RE/dataset/geodata/train.json", type=str, help="The path of the training data.")
    parser.add_argument("--schema_file", default='/RE/dataset/geodata/schema.xlsx', type=str, help="The path of the geological schema.")
    parser.add_argument("--train_mode", type=str, default='random_sorted', choices=['random', 'sorted', 'random_sorted'])
    parser.add_argument("--do_eval", action='store_true', default=1, help="Whether to run eval on the dev set.")
    parser.add_argument("--do_lower_case", action='store_true', default=0, help="Set this flag if you are using an uncased model.")
    parser.add_argument("--eval_test", action="store_true", default=1, help="Whether to evaluate on final test set.")
    parser.add_argument("--eval_with_gold", action="store_true", default=0, help="Whether to evaluate the relation model with gold entities provided.")
    ...
    parser.add_argument("--no_cuda", action='store_true', default=0,
                        help="Whether not to use CUDA when available")
    parser.add_argument('--seed', type=int, default=0,
                        help="random seed for initialization")
    parser.add_argument("--bertadam", action="store_true", help="If bertadam, then set correct_bias = False")

    parser.add_argument("--entity_output_dir", type=str, default="/RE/output/entity_output", help="The directory of the prediction files of the entity model")
    ...
    parser.add_argument('--add_new_tokens', action='store_true', default=0,
